...
`harbor-lang-lexer-grid` is centered on compilers work in Julia. The technical target is: Create a Julia reference implementation for lexer workflows, centered on stream reduction, windowed input fixtures, and late-data behavior checks.
...
The score is deterministic. The current threshold is 160. Risk has a penalty of 5, latency has a penalty of 3, and weight contributes a bonus of 2.
